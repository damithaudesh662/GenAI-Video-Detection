...
def mirror_depthmaps(dataset_root="gen_ai_detector/videos"):
    ...
    for sub in sorted([d for d in root.iterdir() if d.is_dir()], key=lambda p: p.name):
        ...
        for img_file in sorted(sub.iterdir(), key=lambda p: p.name):
            if img_file.is_file() and img_file.suffix.lower() in extensions:
                ...
                out_path = depth_root / root.name / sub.name / out_file
